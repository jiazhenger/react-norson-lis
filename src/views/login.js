...
export default class extends React.Component{
	...
	componentDidMount(){
		...
		cacheApi.then(f => {
			$fn.getCache({
				cache: f.default.CompanySelect, name: 'company_name', id: 'uuid', callback: (data) => {
					if ($fn.hasArray(data)) {
						this.setState({data, selectLoading:false})
                    } else {
                        $http.submit(this, 'company/select', { submitLoading:'selectLoading', loading:false }).then(data=>{
							this.setState({data})
						})
                    }
				}
			...
			<Page contentClassName='fxmc wh login' style={{background:`url(${Bg}) no-repeat center/cover`}}>
				...
				<div className='bcf r10px rel i10' style={{width:500, padding:'25px 40px', boxShadow:'0 0 10px #bbb'}}>
					...
					<Form onSubmit={this.onSubmit} init={form => this.form = form}>
						<Item name='company' rules={[{ required:true, message:'公司不能为空'}]}><Select disabled={loading} loading={selectLoading} size={size} p='选择分公司' bordered={false} data={data} idStr='uuid' nameStr='company_name' onChanged={v=>this.form.setFieldsValue({ 'company':v })}/></Item>
